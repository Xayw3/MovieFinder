...
const MoviesPage = () => {
  ...
  const searchMovies = async (currentPage: number = 1) => {
    if (inputValue) {
      const moviesData = await axios.get(
        `https://api.themoviedb.org/3/search/${location.pathname}?api_key=433e58e14ddff9586a5b1f8d7895559f&query=${inputValue}&page=${currentPage}`,
      );
